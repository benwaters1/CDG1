...
def _cleanup():
    ...
    # And anything else already waiting on the owner. The empty-state and
    # "exactly one thing" checks below assert on the whole queue, so a single
    # unapproved invoice sitting in whoever's development database is enough to
    # fail them there and pass everywhere else. This runs against a throwaway
    # copy — the real database is untouched — so clearing it is free.
    for table in ("expenses", "leave_requests", "timesheet_corrections"):
        conn.execute(f"DELETE FROM {table} WHERE status = 'pending'")
    conn.commit()
    conn.close()
